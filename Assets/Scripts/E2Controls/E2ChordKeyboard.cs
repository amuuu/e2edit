...
public sealed partial class E2ChordKeyboard : VisualElement
{
    ...
    // Active range
    int _baseOctave = 3;
    int BaseNote => (_baseOctave + 1) * 12 + 1;
    ...
    {
        if (_note1 == note) _note1 = 0;
        if (_note2 == note) _note2 = 0;
        if (_note3 == note) _note3 = 0;
        if (_note4 == note) _note4 = 0;
        CompactChord();
    }
    ...
    {
        var i = 0;
        Span<int> chord = stackalloc int[]{0, 0, 0, 0};
        if (_note1 > 0) chord[i++] = _note1;
        if (_note2 > 0) chord[i++] = _note2;
        if (_note3 > 0) chord[i++] = _note3;
        if (_note4 > 0) chord[i++] = _note4;
        _note1 = chord[0];
        _note2 = chord[1];
        _note3 = chord[2];
        _note4 = chord[3];
